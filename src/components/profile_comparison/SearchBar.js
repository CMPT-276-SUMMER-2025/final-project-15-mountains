            ...
            {input && (
                <button
                    ...
                    className="absolute flex items-center justify-center rounded right-3 w-6 h-6
                               hover:cursor-pointer hover:bg-gray-100 dark:hover:bg-muted transition"
                    >
                <span className="text-lg text-gray-800 dark:text-foreground">&times;</span>
                </button>
            )}

            {resultsVisible && results.length > 0 && (
                <ul className="absolute top-full left-0 mt-1 bg-white w-full border rounded
                               dark:bg-popover dark:text-popover-foreground shadow z-1">
                    {results.map((user) => (
                        <li
                            key={user.id}
                            onMouseDown={() => handleSelect(user)}
                            className="cursor-pointer rounded px-3 py-2 hover:bg-gray-100 dark:hover:bg-muted"
                        >
